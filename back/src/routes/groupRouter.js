...
// 로그인한 사용자 정보 불러오기
groupRouter.get('/users', async function (req, res, next) {
  try {
...
// 사용자 정보 수정
groupRouter.patch('/users/:userId', async function (req, res, next) {
  try {
...
//사용자 삭제
groupRouter.delete('/users/:userId', async function (req, res, next) {
  try {
    // params로부터 id를 가져옴
    const { userId } = req.params;
...
//수정, 삭제 전 현재 비밀번호 확인
groupRouter.post('/users/currentPassword', async function (req, res, next) {
  try {
    // req (request) 에서 데이터 가져오기
    const userId = req.currentUserId;
    const password = req.body.password;
